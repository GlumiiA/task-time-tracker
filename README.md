...
- для генерации идей тестовых сценариев;
- для помощи с формулировками в README;
- для генерации boilerplate-кода.

На этапе проектирования я советовалась по поводу логики создания записи времени. Сначала было две интерпретации задачи: создавать запись только тогда, когда заявка уже находится в `DONE`, или фиксировать время в процессе работы над задачей. В первом случае предполагается, что задачи небольшие и в конце по ним достаточно оставить общий итог. Во втором — что задачи могут быть более объемными, и важно иметь возможность поэтапно фиксировать выполненную работу, а учет времени вести в человеко-часах, а не в длительности одного этапа. В итоге я выбрала второй вариант, потому что он лучше соответствовал бизнес-требованиям.

На этапе написания кода, после реализации уровня persistence и проработки эндпоинтов, мне нужно было писать много похожей логики для DTO, обработки исключений и маппинга. Для этого я использовала Copilot, чтобы ускорить генерацию boilerplate-кода.

На этапе тестирования я проверяла покрытие тестами и использовала ИИ для проработки всех сценариев.

Также ИИ использовался для написания минимального UI, чтобы упростить ручную проверку помимо Postman, и для более лаконичной и понятной формулировки README.

Артефакты взаимодействия с ИИ можно посмотреть в папке `docs/ai-notes`. Там есть скриншоты основных шагов и промты, которые использовались для получения рекомендаций и формулировок.